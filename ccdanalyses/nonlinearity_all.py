...
def get_txt_info(a_file, data):
    ...
    data["TXT_DIFF_CURRENT_MEDIAN60"] = np.percentile(np.abs(dc[start_ind_p:stop_ind_n]), 60)
    data["TXT_DIFF_CURRENT_MEDIAN70"] = np.percentile(np.abs(dc[start_ind_p:stop_ind_n]), 70)
    data["TXT_DIFF_CURRENT_MEDIAN80"] = np.percentile(np.abs(dc[start_ind_p:stop_ind_n]), 80)
    data["TXT_DIFF_CURRENT_MEDIAN90"] = np.percentile(np.abs(dc[start_ind_p:stop_ind_n]), 90)
