...
def plot_overscan(overscan, fl, TITLE, OUT_DIR):
	...
	for i,f in enumerate(fl):
		...
		gs = gridspec.GridSpecFromSubplotSpec(1, 2, wspace=0, subplot_spec=gs0[f.dev_index])
		ax2 = plt.subplot(gs[0, 0])
		for j in range(9,17):
			plt.plot(overscan[i, j-1] +500*(j-8), label='seg' + str(j+1))
		plt.legend(fontsize=6,loc='upper center', ncol=4)
		if(x != 0):
			ax2.set_yticklabels([])
		  
		plt.grid()
		plt.xlim(0,2100); plt.ylim(0,4500)
...
def plot_overscan_diff(overscan, fl, TITLE, OUT_DIR):
	...
	for i,f in enumerate(fl):
		...
		ax2 = plt.subplot(gs[0, 0])
		for j in range(9,17):
			plt.plot(overscan[i, j-1] - overscan[i, 15] +500*(j-8), label='seg' + str(j+1))
		plt.legend(fontsize=6,loc='upper center', ncol=4)
		if(x != 0):
			ax2.set_yticklabels([])
		...
		ax1 = plt.subplot(gs[0, 1])
		for j in range(1,9):
			plt.plot(overscan[i, j-1] - overscan[i, 7] +500*j, label='seg' + str(j-1))
		plt.legend(fontsize=6,loc='upper center', ncol=4)
		if(x != 2):
			ax1.set_yticklabels([])
		if(x == 2):
...
def plot_mean_std_stddelta(m, n, nd, fl, TITLE, OUT_DIR):
	...
	for i,f in enumerate(fl):
		...
		ax1.legend(lns, labs, bbox_to_anchor=(0., 1.07, 1., .102), fontsize='small', ncol=3, numpoints=1, loc=9)
...
def plot_histogram_mean(m, TITLE, OUT_DIR):
	...
	for bin_num in arange(10, 100, 10):
	...
	savefig(OUT_DIR + TITLE +'_mean_histo.png')
	plt.close(fig)
...
def plot_histogram_all_one_binning(m, n, nd, TITLE, OUT_DIR, bin_num = 45, num_ccd = 9, omit_REBs = [], read_REBs = set([0,1,2])):
	from matplotlib.patches import Rectangle

	if num_ccd != len(read_REBs)*3:
		print "ERROR! num_ccd = %i while number of REBs being read is %i." % (num_ccd, len(red_REBs))
		return "\n"
	...
	for REB in omit_REBs:
		...
		if omit_REBs:
			...
			for REB in read_REBs:
				i+=1
				if REB not in omit_REBs: continue
	...
	if len(n_no_dead):
		n_mean, n_median, n_std = np.mean(n_no_dead), np.median(n_no_dead), np.std(n_no_dead)
	else: n_mean, n_median, n_std = 0, 0, 0
	if len(m_no_dead):
		m_mean, m_median, m_std = np.mean(m_no_dead), np.median(m_no_dead), np.std(m_no_dead)
	else: m_mean, m_median, m_std = 0, 0 ,0
	if len(nd_no_dead):
		nd_mean, nd_median, nd_std = np.mean(nd_no_dead), np.median(nd_no_dead), np.std(nd_no_dead)
	else: nd_mean, nd_median, nd_std = 0, 0, 0

	bin_num_lin = 4* bin_num / 5
	bin_num_log = 1* bin_num / 5
	bins_lin = np.linspace(0, 30, bin_num_lin)
	val_max = max(max(n_all), max(nd_all))
	if val_max <= 30: val_max = 50
	...
	textstr1 = '$\mu=%.0f$\n$\mathrm{median}=%.0f$\n$\sigma=%.0f$'%(m_mean, m_median, m_std)
...
def plot_summary(data, run, OUT_DIR, SUPTITLE = "Runs comparison"):
	...
	ax1.errorbar(x, data[:,0], marker = 'o', color='darkgreen', yerr=data[x,2], linestyle='None')
	...
	ax2.errorbar(x, data[:,3], marker = 'o', color='darkblue', yerr=data[x,5], linestyle='None')
	...
	ax3.errorbar(x, data[:,6], marker = 'o', color='darkred', yerr=data[x,8], linestyle='None')
...
def plot_cor_all(a, fl, TITLE, OUT_DIR):
	...
	plt.xticks(seg)
	plt.yticks(seg)
	#for i in range(0,num_ccd*16,48):
	#    axhline(i, color='black')
	#    axvline(i, olor='black')
	for i,f in enumerate(fl):
		plt.text(-10*r,8+16*i, f.dev_name, size=15, verticalalignment='center')
		
	widthB = 54 / len(fl)
	widthB = str(widthB)
	
	for i in np.arange(0,len(fl),3):
		REB = 'REB' + fl[i].dev_name[0:1]
		plt.annotate(REB, xy=(-11*r, 24+i*16), xytext=(-18*r, 24+i*16), xycoords='data', fontsize=20, annotation_clip=False,
				 ha='center', va='center',arrowprops=dict(arrowstyle='-[, widthB=%s, lengthB=1.5' % widthB, lw=2.0))
